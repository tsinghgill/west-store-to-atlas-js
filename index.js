...
exports.App = class App {
  ...
  anonymize(records) {
    records.forEach((record) => {
      console.log(`Inside Anonymize record:`, record)
      ...
      console.log(`record after setting storeId:`, record)
    });
  ...
  async run(turbine) {
    ...
    let source = await turbine.resources("west-store-mongo");
    ...
    // Identify the upstream datastore with the `resources` function
    // Replace `source_name` with the resource name configured on Meroxa
    let destination = await turbine.resources("mongo-atlas");
